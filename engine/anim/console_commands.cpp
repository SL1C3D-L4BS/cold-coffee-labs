// engine/anim/console_commands.cpp — Phase 13 (ADR-0039).

#include "engine/anim/console_commands.hpp"
...
AnimationWorld* s_world = nullptr;
...
void cmd_pause(void*, std::span<const std::string_view> argv, console::ConsoleWriter& out) {
    ...
    out.write_line(s_world->paused() ? "anim paused" : "anim running");
}
...
void cmd_step(void*, std::span<const std::string_view> argv, console::ConsoleWriter& out) {
    ...
    char buf[64];
    std::snprintf(buf, sizeof(buf), "stepped %u anim frames", n);
    out.write_line(buf);
}
...
void cmd_hash(void*, std::span<const std::string_view>, console::ConsoleWriter& out) {
    ...
    char buf[64];
    std::snprintf(buf, sizeof(buf), "anim.hash = 0x%016llX",
                  static_cast<unsigned long long>(h));
    out.write_line(buf);
}

std::uint32_t parse_debug_flag(std::string_view tok) noexcept {
    using DF = AnimationWorld::DebugFlag;
    auto v = [](DF f) { return static_cast<std::uint32_t>(f); };
    if (ieq(tok, "skeletons"))   return v(DF::Skeletons);
    if (ieq(tok, "ik_goals"))    return v(DF::IKGoals);
    if (ieq(tok, "root_motion")) return v(DF::RootMotion);
    if (ieq(tok, "morph"))       return v(DF::Morph);
...
void cmd_debug(void*, std::span<const std::string_view> argv, console::ConsoleWriter& out) {
    ...
    char buf[64];
    std::snprintf(buf, sizeof(buf), "anim.debug = 0x%08X", mask);
    out.write_line(buf);
}

} // namespace

void register_anim_console_commands(console::ConsoleService& svc, AnimationWorld& world) {
    s_world = &world;
    {
        console::Command cmd{}; cmd.name = "anim.pause"; cmd.help = "Toggle animation simulation pause.";
        cmd.fn = &cmd_pause; cmd.flags = console::kCmdDevOnly;
        (void)svc.register_command(std::move(cmd));
    }
    {
        console::Command cmd{}; cmd.name = "anim.step"; cmd.help = "Advance the animation world N fixed steps.";
        cmd.fn = &cmd_step; cmd.flags = console::kCmdDevOnly;
        (void)svc.register_command(std::move(cmd));
    }
    {
        console::Command cmd{}; cmd.name = "anim.hash"; cmd.help = "Print the deterministic hash of all live poses.";
        cmd.fn = &cmd_hash; cmd.flags = console::kCmdDevOnly;
        (void)svc.register_command(std::move(cmd));
    }
    {
        console::Command cmd{}; cmd.name = "anim.debug";
        cmd.help = "Set the animation debug draw flag mask (skeletons|ik_goals|root_motion|morph|all|off).";
