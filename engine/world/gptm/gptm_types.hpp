#pragma once
// engine/world/gptm/gptm_types.hpp — Phase 20-A GPTM core types (Greywater Planetary Topology Mesher).
// "Planetary" here is the historical NAME of the tiling algorithm; the engine only simulates bounded
// arena chunks per docs/01 §2.6 (Nine Circles of Hell are arenas, not planets). See ADR-0090.
...
///
/// `local_pos` is CHUNK-LOCAL, POST-FLOATING-ORIGIN. Absolute world-space lives in `GptmTile::world_bounds`
/// as `Aabb3_f64`. This is the only sanctioned `float` position pathway per CLAUDE.md NN #17 and
/// docs/01_CONSTITUTION_AND_PROGRAM.md §2.6 ("`float` only for local-to-chunk math and GPU attributes after
/// floating-origin subtraction"). Do NOT propagate these values to world-space APIs without reapplying the
/// chunk origin offset (see `engine/world/origin/floating_origin.hpp`).
struct GptmVertex {
    float         local_pos[3]{};  ///< Chunk-local metres, post floating-origin.
