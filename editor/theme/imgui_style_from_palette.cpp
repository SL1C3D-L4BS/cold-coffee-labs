...
void apply_palette_to_imgui_style(const Palette& palette, ImGuiStyle& style,
    ...
    // Ensure any newly added `ImGuiCol_*` entries get a sane default before we
    // paint the full semantic palette on top.
    ...
    const ImVec4 frame_hov     = lerp(panel, accent, 0.28f);
    const ImVec4 frame_act     = lerp(panel, accent, 0.45f);
    const ImVec4 title         = lerp(bg, panel, 0.35f);
    const ImVec4 title_act     = lerp(panel, accent, 0.12f);
    ...
    col[ImGuiCol_PopupBg]               = mul_alpha(panel, viewports_enable ? 1.f : 0.98f);
    ...
    col[ImGuiCol_TitleBgCollapsed]      = mul_alpha(title, 0.85f);
    col[ImGuiCol_MenuBarBg]             = panel;
    ...
    col[ImGuiCol_ButtonHovered]         = mul_alpha(accent, 0.45f);
    col[ImGuiCol_ButtonActive]          = mul_alpha(accent_hi, 0.75f);
    col[ImGuiCol_Header]                = mul_alpha(accent, 0.35f);
    col[ImGuiCol_HeaderHovered]         = mul_alpha(accent, 0.55f);
    ...
    col[ImGuiCol_DockingPreview]        = mul_alpha(accent, 0.45f);
    col[ImGuiCol_DockingEmptyBg]        = bg;
    col[ImGuiCol_PlotLines]             = accent;
    col[ImGuiCol_PlotLinesHovered]      = warn;
    col[ImGuiCol_PlotHistogram]        = good;
    col[ImGuiCol_PlotHistogramHovered] = warn;
    col[ImGuiCol_TableHeaderBg]         = lerp(panel, accent, 0.08f);
    col[ImGuiCol_TableBorderStrong]     = lerp(sep, accent, 0.15f);
    ...
    (void)bad; // reserved for future destructive-tinted widgets
