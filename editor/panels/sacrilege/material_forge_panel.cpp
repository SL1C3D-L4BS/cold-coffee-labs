...
std::optional<std::filesystem::path> MaterialForgePanel::cached_albedo_path(
...
void MaterialForgePanel::draw_material_cell(gw::editor::EditorContext& ctx, const std::string& id,
    ...
    if (ImGui::BeginChild("##cell", {100.f, 132.f}, true, ImGuiWindowFlags_NoScrollbar)) {
        ...
        const char* badge = has_preview_file ? "OK" : "NO ALB";
        const ImVec4 badge_col =
...
void MaterialForgePanel::draw_library_grid(gw::editor::EditorContext& ctx) {
...
void MaterialForgePanel::on_imgui_render(gw::editor::EditorContext& ctx) {
    ...
    ImGui::TextDisabled("AmbientCG (CC0) — library browse. Full PBR: sync + cook (see docs/13).");
