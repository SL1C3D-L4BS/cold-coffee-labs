...
TEST_CASE("Job submission with template interface") {
    Scheduler scheduler(2);
    
    std::atomic<bool> job_executed{false};
    
    // Submit job using template interface with no arguments
    // We'll use a dummy parameter to match the template
    JobHandle handle = scheduler.submit<int>(
        JobPriority::Normal,
        [&job_executed](int) {
            job_executed = true;
        },
...
TEST_CASE("Multiple job submission with different priorities") {
    ...
    // Submit jobs with different priorities (using template interface)
    JobHandle handle1 = scheduler.submit<int>(JobPriority::Low, [&]() {
        priorities.push_back(JobPriority::Low);
        execution_order.fetch_add(1);
    }, 1);
    
    JobHandle handle2 = scheduler.submit<int>(JobPriority::Critical, [&]() {
        priorities.push_back(JobPriority::Critical);
        execution_order.fetch_add(1);
    }, 2);
    
    JobHandle handle3 = scheduler.submit<int>(JobPriority::High, [&]() {
...
TEST_CASE("WaitGroup functionality") {
    ...
    // Add jobs to wait group (using template interface)
    JobHandle handle1 = scheduler.submit<int>(JobPriority::Normal, [&]() {
        completed_jobs.fetch_add(1);
    }, 1);
    
    JobHandle handle2 = scheduler.submit<int>(JobPriority::Normal, [&]() {
        completed_jobs.fetch_add(1);
    }, 2);
    
    JobHandle handle3 = scheduler.submit<int>(JobPriority::Normal, [&]() {
        completed_jobs.fetch_add(1);
    }, 3);
...
TEST_CASE("Parallel for loop functionality") {
    ...
    // Use parallel_for to process 100 items
    scheduler.parallel_for(100, [&](uint32_t index) {
...
TEST_CASE("Job system stress test") {
    ...
    // Submit many jobs (using template interface)
    for (uint32_t i = 0; i < NUM_JOBS; ++i) {
        JobHandle handle = scheduler.submit<int>(JobPriority::Normal, [&completed_jobs](int) {
            completed_jobs.fetch_add(1);
        }, i);
...
TEST_CASE("Job system integration test") {
    ...
    // Phase 1: Initial job (using template interface)
    JobHandle handle1 = scheduler.submit<int>(JobPriority::High, [&]() {
        phase.store(1);
    }, 1);
    ...
    // Phase 3: Final job (using template interface)
    JobHandle handle4 = scheduler.submit<int>(JobPriority::Low, [&]() {
        phase.store(2);
    }, 4);
