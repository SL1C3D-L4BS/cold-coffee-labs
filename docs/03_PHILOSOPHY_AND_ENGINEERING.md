...
**Status:** Canonical · Binding at review · Last revised 2026-04-22  
**Precedence:** L2
...
Targeting Vulkan 1.2 with opportunistic 1.3 features is a commitment. It means **every player on an RX 580 or better can run *Sacrilege* at 1080p / 144 FPS** for Tier A content (L1), while **engine reference harnesses** may still use **≥ 60 FPS** rows in `docs/05_RESEARCH_BUILD_AND_STANDARDS.md` §1.4 — do not cite those as the flagship ship bar. We don't exclude mid-range hardware to chase ray tracing or mesh shaders.
...
The editor is a native C++ executable that statically links `greywater_core`. It uses Dear ImGui with docking and viewports. There is no managed runtime, no C# bridge, no Python **in the editor binary or runtime link**, no custom DSL for tool widgets. (Offline Python scripts under `tools/` for CI/docs: `docs/01_CONSTITUTION_AND_PROGRAM.md` §2.2.1.) Anything a gameplay programmer can do, a tools programmer can do, and vice versa.
...
- **We will not add a scripting VM** (no Lua, no JavaScript, no embedded Python **in shipped binaries**). Visual scripting is a typed text IR; gameplay extension is hot-reloaded C++. Offline Python in `tools/` only: `01` §2.2.1.
...
- **We will not chase RTX-tier features that shut out RX 580 players.** Every Tier A *Sacrilege* visual feature must run at **1080p @ 144 FPS** on an RX 580 (L1). Engine reference harnesses may use separate 60 FPS budgets — `05` §1.4.
