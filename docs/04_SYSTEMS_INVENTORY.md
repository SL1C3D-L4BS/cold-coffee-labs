...
**Program context:** Phases 19–23 are **Blacklake + *Sacrilege***. Rows describe **engine modules** the flagship consumes via public API. Product behavior: `docs/07_SACRILEGE.md`.
...
Every subsystem below has a **tier**, an **owning phase** (see `02_ROADMAP_AND_OPERATIONS.md` — phases 1–25; Phase 25 is LTS), an **owning module** (directory in the repo), and a **concrete library decision** or a *custom* label.
...
## 2a. *Sacrilege* / Blacklake world rendering (Tier A — game-facing)

These systems target ***Sacrilege*** debut content and shared Blacklake tooling, but use the engine's public API. They live in `engine/world/` as engine capabilities the program consumes (see `docs/01_CONSTITUTION_AND_PROGRAM.md`, `docs/02_ROADMAP_AND_OPERATIONS.md`, `docs/07_SACRILEGE.md`).
